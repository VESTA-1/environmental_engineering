...
def original_data_process(data):
    ...
    temp_name = []
    temp_result = []
    y = data.loc['總氯化乙烯同度（μmole/L)'][:90]
    y.reset_index()
    # index = pd.DataFrame(list(data.columns))
    feature = data.loc[list('ASV{:0=3}'.format(i) for i in range(1, 727))]
    ...
    result.to_csv('data/data2.csv', encoding='utf-8_sig', index=False)
    # print(result)
    # exit()
    ...
    train_data.to_csv('data/train2.csv', encoding='utf-8_sig', index=False)
    return train_data
...
origin_data = read_data('data/original_data_2.xlsx')
# print(origin_data)
